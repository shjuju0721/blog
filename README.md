...
**Next.js + Supabase** 로 만든 개인 블로그 사이트입니다.
...
- **[Supabase](https://supabase.com/)** (PostgreSQL + 자동 생성 REST API)
...
3. 이 저장소의 `supabase/schema.sql` 내용을 붙여넣고 **Run** 실행
   → `posts` 테이블, `category` 컬럼, `updated_at` 자동 갱신 트리거, 보안 정책(RLS)이 생성됩니다.
...
├─ src/
│  ├─ app/
│  │  ├─ layout.tsx                 # 공통 레이아웃 (헤더 + 좌측 카테고리 사이드바)
...
│  │  ├─ PostForm.tsx               # 작성/수정 공용 폼 (카테고리 선택 포함)
...
├─ supabase/
│  ├─ schema.sql                    # 전체 DB 스키마 + RLS 정책 (신규 설치용)
│  └─ migrations/
...
이미 운영 중인 DB(예: `category` 컬럼이 없던 초기 버전)를 업데이트하려면,
`supabase/migrations/001_add_category.sql` 내용을 Supabase **SQL Editor** 에서 한 번 실행하세요.
(신규로 `schema.sql` 을 실행하는 경우엔 이미 포함되어 있어 별도 작업이 필요 없습니다.)
